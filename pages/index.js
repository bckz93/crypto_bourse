...
export default function Home({ res }) {
  return (
    <Layout page="Crypto Watch - Accueil">
      <ul className="grid grid-cols-3 gap-5 justfy-around py-10">
            ...
            <Link href={`/${crypto.id}`}>
              <a className="rounded-md">
                ...
                <h3 className=" font-bold text-2xl mb-4">
                  {parseFloat(crypto.price).toFixed(2)} EUR
                </h3>
                <p>
                  1 jour:{" "}
...
// Rendu coter serveur (serveur side rendered)
export async function getStaticProps(context) {
  try {
    const res = await fetch(
      " https://api.nomics.com/v1/currencies/ticker?key=5789e869854e7df90e31355fdd1b799ac634d2bb&ids=BTC,ETH,AAVE,XRP,BCH,XLM,LTC,EOS,NEO&interval=1d,30d,365d&convert=EUR"
    ).then((res) => res.json());
